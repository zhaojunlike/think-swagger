...
class SwaggerAssetRegister extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        if ($input->hasOption('config')) { // 安装配置文件
            ...
            $path = App::getConfigPath() . DS . 'swagger.php';
            copy($src, $path);
        ...
        $path = $option ?: App::getRootPath() . 'public' . DS . 'static' . DS;

        //不是根目录 放到默认目录下面的相对目录,
        if (DS != substr($option, 0, 1)) {
            $path = App::getRootPath() . 'public' . DS . 'static' . DS . $option;
        }
        $path .= 'swagger-ui' . DS;
        if (is_dir($path)) {
            $src = __DIR__ . DS . '..' . DS . 'assets' . DS . 'swagger-ui' . DS . 'dist';
